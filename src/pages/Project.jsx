...
const Project = () => {
    ...
    const { getProject, project, load, handleModalTask, alert, submitTaskSProject, deleteTaskSProject, editTaskSProject, changeTaskSProject } = useProjects()
    ...
    useEffect(() =>{
        ...
        socket.on('task deleted', taskDeleted => {
            if(taskDeleted.project === project._id){
                deleteTaskSProject(taskDeleted)
            }
        })
        socket.on('task updated', taskUpdated => {
            if(taskUpdated.project._id === project._id){
                editTaskSProject(taskUpdated)
            }
        })
        socket.on('new task', newStateTask => {
            if(newStateTask.project._id === project._id){
                changeTaskSProject(newStateTask)
            }
        })
    })
        ...
        :(
            <>
                ...
                <button onClick={handleModalTask} type="button" className="flex gap-2 items-center text-sm px-5 py-3 w-full md:w-auto rounded-lg uppercase font-bold bg-sky-400 hover:bg-sky-600 text-white text-center mt-5">
                    ...
                    <p className="font-bold text-xl mt-10">Task of Project</p>
                    ...
                    <div className="bg-white shadow rounded-lg mt-10">
                        {project.tasks?.length ?
                            ...
                            <Task  
                                ...
                                task={task}
                            />))
                            : <p className="text-center my-5 p-10">Dont has Task in this project</p>}
                        ...
                        <>
                            ...
                            <div className="bg-white shadow rounded-lg mt-10">
                                {project.collaborators?.length ?
                                    ...
                                    : <p className="text-center my-5 p-10">Dont has Collaborators in this project</p>}
